...
class _AddProfileState extends State<AddProfile> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colormanager.scaffold,
        title: const Text('Add Profile', style: TextStyle()),
        centerTitle: true,
      ),
                        ...
                        child: Stack(
                          children: [
                            ...
                            Positioned(
                              top: 120,
                              left: 145,
                              child: Icon(Icons.add_to_photos, size: 40),
                            )
                          ],
                        ),
                      ),
                        ...
                        child: BlocBuilder<DateOfBirthBloc, DateOfBirthState>(
                          builder: (context, state) {
                            if (state is DateOfBirthSelectedState) {
                              _datofbirthController.text = state.dateOfBirth;
                            }
                            return DateofBirth(
                              value: (value) {
                                if (value!.isEmpty) {
                              ...
                              onChange: (value) {
                                if (value != null) {
                                  BlocProvider.of<GenderBloc>(context).add(
                                    GenderSelected(genderselectedvalue.toString()),
                                  );
                                }
                              },
                      ...
                      GestureDetector(
                        onTap: () {
                          ...
                          if (_formkey.currentState!.validate() && imageUrl.isNotEmpty) {
                            ...
                            final mbobilecontroller = int.parse(_mbobilecontroller.text);
                            
                            BlocProvider.of<AddUserBloc>(context).add(AddUserClick(
                              name: name,
                              age: age,
                              dob: dob,
                              imageUrl: imageUrl,
  ...
  void _clearForm(BuildContext context) {
    ...
    _mbobilecontroller.clear();
    imageUrl = '';
    BlocProvider.of<ImageAddingBloc>(context).add(ImageCleared());
    BlocProvider.of<DateOfBirthBloc>(context).add(DateOfBirthCleared());
    BlocProvider.of<GenderBloc>(context).add(GenderCleared());
  }
